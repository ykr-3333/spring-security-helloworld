...
public class MD5Util {
	...
	private static String digest(String inStr, int rang) {
		...
		try {
			md5 = MessageDigest.getInstance("MD5"); // 鍙栧緱绠楁硶
		} catch (Exception e) {
			e.printStackTrace();
			return "";
		}
		...
		byte[] md5Bytes = md5.digest(byteArray); // 鍔犲瘑
		...
		if (rang == 32) {
			...
			return hexValue.toString().substring(8, 24);// 杞崲涓�32浣嶅瓧绗︿覆
		}
	}

	public static void main(String args[]) {
		String s = new String("123456");  //e10adc3949ba59abbe56e057f20f883e
